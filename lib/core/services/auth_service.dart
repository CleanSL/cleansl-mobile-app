...
class AuthService {
  ...
  Future<void> signUpResident({required String fullName, required String mobile, required String email, required String password}) async {
    ...
    if (user != null) {
      await _supabase.from('users').insert({'id': user.id, 'role': 'resident', 'full_name': fullName, 'phone_number': mobile, 'email': email});

      await _supabase.from('resident_profiles').insert({'user_id': user.id, 'total_points': 0});
    }
  }
  ...
  Future<void> signInResident({required String identifier, required String password}) async {
    ...
    if (identifier.contains('@')) {
      final response = await _supabase.from('users').select('email').eq('email', identifier).eq('role', 'resident').maybeSingle();
  ...
  Future<void> signInWithGoogle() async {
    ...
    if (user != null) {
      final existingUser = await _supabase.from('users').select('id').eq('id', user.id).maybeSingle();

      if (existingUser == null) {
        await _supabase.from('users').insert({'id': user.id, 'role': 'resident', 'full_name': user.userMetadata?['full_name'] ?? 'Google User', 'email': user.email ?? ''});
